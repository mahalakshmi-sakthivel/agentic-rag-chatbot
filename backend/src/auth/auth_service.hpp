 ...
 * It coordinates: PasswordHasher, TokenService, SessionManager, and the user repository.
 *
 * The service NEVER:
 *   - Logs passwords
...
public:
     ...
     * @return LoginResult with JWT on success, generic failure message on failure.
     *
     * Error messages do NOT reveal whether the email exists (prevents enumeration).
     */
     ...
     */
    bool logout(const IdentityContext& identity);
     ...
     */
    std::optional<UserInfo> get_current_user(const IdentityContext& identity) const;
...
private:
    ...
    std::optional<std::string> get_password_hash(const std::string& user_id) const;
