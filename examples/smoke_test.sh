...
echo
echo "== Checklist item 6: wrong HTTP methods on real routes are rejected =="
# NOTE: this repo's routes are registered per-method (registerHandler with an
# explicit {Get}/{Post} filter), so an unmatched method on a real path falls
# through to the same setCustom404Page as a truly unknown route — Drogon
# does not distinguish "wrong verb" from "no such path" here. We assert on
# the envelope + "not a success status" rather than hardcoding 404 vs 405,
# since that exact status is a Drogon-version/config detail worth confirming
# directly against your build rather than trusting this comment.
check_reject() {
  local desc="$1" method="$2" path="$3"
  local status
  status=$(curl -sS -o /tmp/resp.json -w "%{http_code}" -X "$method" "$BASE_URL$path")
  if [ "$status" -ge 400 ] && jq -e '.success == false' /tmp/resp.json > /dev/null 2>&1; then
    echo "  OK   $desc (status $status, enveloped error)"
    PASS=$((PASS + 1))
  else
    echo "  FAIL $desc (status $status) — check router.cpp / Drogon's method-mismatch behavior for this route"
    FAIL=$((FAIL + 1))
  fi
}
check_reject "POST /v1/health is rejected" POST /v1/health
check_reject "GET /v1/query is rejected" GET /v1/query
check_reject "GET /v1/data/upload is rejected" GET /v1/data/upload

echo
echo "== Checklist item 3: unsupported multiple-file upload is rejected =="
status=$(curl -sS -o /tmp/resp.json -w "%{http_code}" -X POST "$BASE_URL/v1/data/upload" \
  -H "$DEV_AUTH_HEADER" \
  -F "file=@/tmp/smoke_test.pdf" -F "file=@/tmp/smoke_test.pdf" -F "file_type=pdf")
check_status "two files in one request returns 400" 400 "$status"
check_json "error.code is VALIDATION_ERROR" '.error.code' 'VALIDATION_ERROR'

echo
echo "== Checklist item 3: unexpected multipart field is rejected =="
status=$(curl -sS -o /tmp/resp.json -w "%{http_code}" -X POST "$BASE_URL/v1/data/upload" \
  -H "$DEV_AUTH_HEADER" \
  -F "file=@/tmp/smoke_test.pdf" -F "file_type=pdf" -F "not_a_real_field=hack")
check_status "unexpected field returns 400" 400 "$status"
check_json "error.details.field is 'not_a_real_field'" '.error.details.field' 'not_a_real_field'

echo
echo "== Checklist item 3: empty (0-byte) file upload is rejected =="
: > /tmp/smoke_test_empty.pdf
status=$(curl -sS -o /tmp/resp.json -w "%{http_code}" -X POST "$BASE_URL/v1/data/upload" \
  -H "$DEV_AUTH_HEADER" \
  -F "file=@/tmp/smoke_test_empty.pdf" -F "file_type=pdf")
check_status "empty file returns 400" 400 "$status"
check_json "error.code is VALIDATION_ERROR" '.error.code' 'VALIDATION_ERROR'
