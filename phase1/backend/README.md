...
A two-stage `Dockerfile` packages the exact build/test steps above — it
does **not** change the contract, API surface, schemas, or seams (see
`DOCKER_VERIFICATION.md` for the full "what changed vs. what didn't"
breakdown).
...
The build stage compiles the server and GoogleTest suite and runs
`ctest --output-on-failure` as part of the build — if any of the 40 unit
tests fail, `docker build` fails and no image is produced. The runtime
stage is a slim `ubuntu:24.04` with only the shared libraries the compiled
binary actually links against, running as a non-root user.
...
All Phase 1 environment variables (`BACKEND_PORT`, `FILE_STORAGE_PATH`,
`LOG_LEVEL`, `ENVIRONMENT`, `MAX_UPLOAD_SIZE_MB`, `AUTH_DEV_BYPASS`) are
read from the environment — see `.env.example` at the repo root. None are
hardcoded in the image; the image's own defaults mirror `.env.example` but
with `ENVIRONMENT=production` and `AUTH_DEV_BYPASS=false`, so a real
deployment has to opt into dev bypass explicitly rather than inherit it.
...
The image also declares a `HEALTHCHECK` that polls this same endpoint every
30s, so `docker ps` / `docker compose ps` shows `healthy` once the server is
up.

**Run the required tests against the running container:**

```bash
./examples/smoke_test.sh
```

(The 40 GoogleTest unit tests already ran as part of `docker build` above,
per the same "tests are required for verification" step described in
Section 5 — this second run is the live-endpoint smoke test, which needs an
actual running server.)

Full dated verification transcript — exact apt package versions,
`ldd`-derived runtime library list, build/test/container-start/smoke-test
output — is in `DOCKER_VERIFICATION.md` at the repo root.
...
├── CMakeLists.txt
├── CMakePresets.json
├── BUILD_VERIFICATION.md       # dated, from-scratch build/test/smoke-test transcript
├── Dockerfile                  # two-stage build; runs unit tests during `docker build`
├── .dockerignore
└── data/uploads/                # reserved for a Phase 3 module's own use (gitignored);
                                   # Phase 1 itself no longer writes here — see "Ingestion" seam above

Repo root also has: docker-compose.yml (optional convenience wrapper),
DOCKER_VERIFICATION.md (dated Docker build/test/container transcript),
.env.example (variables both plain-Docker and compose read from).
```
...
| Ingestion | `UploadController::ingestionHandler` (a reassignable `std::function`, same pattern as `queryHandler`) — Phase 1 validates the request and forwards `IngestionRequest` (file bytes + filename + file_type + metadata + `IdentityContext`); Phase 3 returns an `IngestionResult` (document_id + status, or an error code). Phase 1 does **not** write the file to disk itself — see `api/controllers/UploadController.h` | Phase 3 |
...
| POST | `/v1/data/upload` | **Enforced** (401 if missing/invalid) | Real validation (auth, single file, file_type, size, metadata JSON, filename sanitization) + forwards to Phase 3 via `UploadController::ingestionHandler`. With no Phase 3 module linked in yet, the default handler returns a placeholder `document_id`/`status: queued` and does **not** persist the file — see "Seams for later phases" above |
