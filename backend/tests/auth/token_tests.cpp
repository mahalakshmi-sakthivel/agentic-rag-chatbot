...
TEST_CASE_METHOD(TokenFixture, "TokenService: valid token validates successfully", "[token]") {
...
TEST_CASE_METHOD(TokenFixture, "TokenService: token is a non-empty string", "[token]") {
    auto token = svc.create_token(sample_claims());
...
TEST_CASE_METHOD(TokenFixture, "TokenService: expired token returns nullopt", "[token]") {
...
TEST_CASE_METHOD(TokenFixture, "TokenService: tampered signature returns nullopt", "[token]") {
    auto token = svc.create_token(sample_claims());
...
TEST_CASE_METHOD(TokenFixture, "TokenService: token signed with different secret returns nullopt", "[token]") {
...
TEST_CASE_METHOD(TokenFixture, "TokenService: wrong issuer returns nullopt", "[token]") {
    AuthConfig bad_cfg = make_config();
...
TEST_CASE_METHOD(TokenFixture, "TokenService: wrong audience returns nullopt", "[token]") {
    AuthConfig bad_cfg = make_config();
...
TEST_CASE_METHOD(TokenFixture, "TokenService: empty string returns nullopt", "[token]") {
    auto result = svc.validate_token("");
    REQUIRE_FALSE(result.has_value());
}

TEST_CASE_METHOD(TokenFixture, "TokenService: random garbage returns nullopt", "[token]") {
    auto result = svc.validate_token("not.a.jwt.at.all.xxxx");
    REQUIRE_FALSE(result.has_value());
}

TEST_CASE_METHOD(TokenFixture, "TokenService: plain base64 payload (no signature) returns nullopt", "[token]") {
...
TEST_CASE_METHOD(TokenFixture, "TokenService: JWT secret is not embedded in token", "[token]") {
    auto token = svc.create_token(sample_claims());
