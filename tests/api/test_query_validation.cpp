...
// Checklist item 4 — string trimming. These confirm the trimmed value is
// actually propagated to normalizedBody, not just used for the empty-check
// and then discarded.

TEST(QueryValidation, TrimsQueryIntoNormalizedBody)
{
    nlohmann::json body = {
        {"query", "  What was Q3 revenue?  "},
        {"session_id", "123e4567-e89b-42d3-a456-426614174000"}};
    ...
    EXPECT_EQ(result.normalizedBody.at("query").get<std::string>(), "What was Q3 revenue?");
}

TEST(QueryValidation, TrimsSessionIdBeforeUuidCheckAndIntoNormalizedBody)
{
    nlohmann::json body = {
        {"query", "hello"},
        {"session_id", "  123e4567-e89b-42d3-a456-426614174000  "}};
    const auto result = validateQueryRequest(body);
    ASSERT_TRUE(result.valid);
    EXPECT_EQ(result.normalizedBody.at("session_id").get<std::string>(),
              "123e4567-e89b-42d3-a456-426614174000");
}

TEST(QueryValidation, TrimsConversationIdIntoNormalizedBody)
{
    nlohmann::json body = {
        {"query", "hello"},
        {"session_id", "123e4567-e89b-42d3-a456-426614174000"},
    ...
    const auto result = validateQueryRequest(body);
    ASSERT_TRUE(result.valid);
    EXPECT_EQ(result.normalizedBody.at("conversation_id").get<std::string>(),
              "223e4567-e89b-42d3-a456-426614174000");
}

TEST(QueryValidation, TrimsEachDocumentIdIntoNormalizedBody)
{
    nlohmann::json body = {
        {"query", "hello"},
        {"session_id", "123e4567-e89b-42d3-a456-426614174000"},
    ...
    const auto result = validateQueryRequest(body);
    ASSERT_TRUE(result.valid);
    EXPECT_EQ(result.normalizedBody.at("document_ids")[0].get<std::string>(), "doc-1");
    EXPECT_EQ(result.normalizedBody.at("document_ids")[1].get<std::string>(), "doc-2");
}
